...
def donation():
      ...
      if request.method =="GET":
           ...
           if request.form.get("userid") =="":
               ...
               #save the refno in session so we can reetrieve the details on the next page
               session["ref"] = refno
...
def make_payment():
     ...
     if session.get("ref") !=None:
          ref = session["ref"]
          #we then want to get the details of the transacion and display to the user
...
def paystack_landing():
     ...
     if ref ==None:
          return redirect('/donate')
     else: #connect to payment verify
           headers = {"Content-Type": "application/json", "Authorization":"Bearer sk_test_2dac3a70becc7334588ad63219f038c1e2302ca3"}
...
def search_book():
     ...
     toreturn =""
     for r in result:
        toreturn = toreturn + f"<div class='col'><img src='/static/collection/{r.book_cover}' class='img-fluid bk'><div class='deets'><h6><a href='/review/{r.book_id}'>{r.book_title}</a></h6><p><i>{r.catdeets.cat_name}</i></p><p><button class='btn btn-sm btn-warning'{len(r.bookreviews)}>Reviews</button></p></div></div>"
      ...
      if request.method =="GET":
          return render_template("user/loginpage.html")
           ...
           if deets:
                ...
                chk = check_password_hash(hashedpwd,password)#return True/false
           ...
           return "user will be authentcated and logged in"
    ...
    if request.method=="GET":
        return render_template("user/signup.html",signupform=signupform)
         ...
         if  signupform.validate_on_submit():
              ...
              session["user_loggedin"] =True
              return redirect("/dashboard")            #   flash("You have successfully created an account")                      
            #   return redirect("/login")
         else:
            return render_template("user/signup.html",signupform=signupform)
